...
impl<'t, S: TokenSource<'t>> Parser<S> {
    ...
    pub fn parse_simple_string(
        ...
        self.parse_string_content(start_position, Token::StringEnd)
    }
    /// parse a path expression.
    /// assumes the path begin token has already been consumed.
    pub fn parse_path(&mut self, start_position: SourcePosition) -> ParseResult<NixString<'t>> {
        self.parse_string_content(start_position, Token::PathEnd)
    }
    ...
    pub fn parse_multiline_string(
        ...
        let mut str = self.parse_string_content(start_position, Token::StringEnd)?;
    ...
    fn parse_string_content(
        ...
        let next_part = match t.token {
            ...
            Token::BeginInterpol => {
                return self.parse_interpolated_string(
                    position,
                    NixString::from_literal(initial_content, position),
                    end_token,
                );
            }
            tok if tok == end_token => {
                return Ok(NixString::from_literal(initial_content, position));
            }
            _ => return unexpected(t),
        };
